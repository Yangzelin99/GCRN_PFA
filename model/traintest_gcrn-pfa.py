...
import argparse
import logging
from model.utils import StandardScaler, DataLoader, masked_mae_loss, masked_mape_loss, masked_mse_loss
from model.GCRN_PFA import GCRN_PFA
...
def get_model():
    model = GCRN_PFA(num_nodes=args.num_nodes, input_dim=args.input_dim, output_dim=args.output_dim,
...
def evaluate(model, mode):
    with torch.no_grad():
        ...
        if mode == 'test':
            logger.info(
                'Horizon overall: mae: {:.4f}, mape: {:.4f}, rmse: {:.4f}'.format(mean_mae, mean_mape, mean_rmse))
...
def traintest_model():
    model = get_model()
    print_model(model)
    optimizer = torch.optim.NAdam(model.parameters(), lr=args.lr, eps=args.epsilon)
...
parser.add_argument('--trainval_ratio', type=float, default=0.7,
                    help='the ratio of training and validation data among the total')
parser.add_argument('--val_ratio', type=float, default=0.2,
                    help='the ratio of validation data among the trainval ratio')
...
parser.add_argument("--use_curriculum_learning", type=eval, choices=[True, False], default='True',
